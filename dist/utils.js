...
export async function extractProducts() {
    try {
        ...
        const productElements = document.querySelectorAll('[data-testId="grid-item"]');
        productElements.forEach((product, index) => {
            const productLikes = +product.querySelector("button span.web_ui__Text__caption").textContent;
            if (productLikes >= 15) {
...
export function parseProductInfo(altText) {
    const info = {};
...
export function displayProducts(products) {
    ...
    console.log("   which shows trending/featured items, not necessarily the latest listings.");
}
